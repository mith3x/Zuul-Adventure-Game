 ...
 * @version A3 Solution 2024-03-17
 */
...
public class Game {
    ...
    private void createRooms() {
        ...
        Item chair, bar, computer, computer2, tree, chip;
        Beamer beamer;
        ...
        chip = new Item("chip", "a chip", 0.5);

        TransporterRoom transporter = new TransporterRoom("in a transporter room");
        ...
        transporter.setExit("north", outside);
    ...
    private void goRoom(Command command) {
        ...
        if (nextRoom == null) {
            ...
            if (nextRoom.getShortDescription().equals("in the transport")) {
                nextRoom = Room.allRooms.get(random.nextInt(Room.allRooms.size()));
            }
    ...
    private void eat(Command command) {
        if (currentItem.getName().equals("chip")) {
            currentItem = null;
            hungerLevel = 1;
            System.out.println("You have eaten the chip and are no longer hungry. You have no food with you now");
        } else {
     ...
     * whether we really want to take. If we are not carrying an item, take it.
     *
     * @param command The command to be processed
     */
    private void take(Command command) {
        if (!command.hasSecondWord()) {
            ...
            if (currentItem == null) {
                ...
                if (pickedItem != null) {
                    if (hungerLevel <= 5 || item.equals("chip")) {
     ...
     * whether we really want to drop. If we are carrying an item, drop it.
     *
     * @param command The command to be processed
     */
     ...
     * whether we really want to charge. If the beamer is not charged, charge it.
     *
     * @param command The command to be processed
     */
     ...
     * whether we really want to fire. If the beamer is charged, fire it.
     *
     * @param command The command to be processed
     */
